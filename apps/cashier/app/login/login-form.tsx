...
export function CashierLoginForm({ employees }: { employees: { id: string; name: string }[] }) {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center p-4">
        <Card className="w-full max-w-xs text-center">
          ...
          <p className="mb-6 text-sm text-foreground/60">اختر اسمك للمتابعة</p>
          ...
          <div className="space-y-2">
            {employees.map((e) => (
              <button
                ...
                className="flex w-full items-center justify-center rounded-xl border border-border px-4 py-3 text-sm font-semibold transition-colors hover:bg-muted active:scale-[0.98]"
              >
                {e.name}
              </button>
            ))}
            {employees.length === 0 ? (
              <p className="text-sm text-foreground/60">لا يوجد موظفو كاشير مفعّلون — راجع الأدمن لإضافتك</p>
            ) : null}
          </div>
        </Card>
      </main>
  ...
  return (
    <main className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-xs text-center">
        <h1 className="mb-1 text-xl font-bold">أهلًا {selectedEmployee.name}</h1>
        <p className="mb-6 text-sm text-foreground/60">أدخل رمز PIN الخاص بك</p>
        ...
        <form action={formAction}>
          ...
          <div className="mt-4 space-y-2">
            ...
            <button
              ...
              className="w-full text-xs text-foreground/50 underline underline-offset-2"
            >
              ليس أنا — تغيير الاسم
            </button>
          </div>
