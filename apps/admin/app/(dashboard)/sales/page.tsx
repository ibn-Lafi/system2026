...
// نظرة عامة على وحدة "المبيعات" = لوحة مؤشرات سريعة (KPIs + رسوم بيانية)
// بفترات جاهزة فقط — اختيار فترة مخصّصة وتفاصيل كل تقرير على حدة (جداول،
// تنزيل ملف) بصفحة "التقارير" المخصّصة لذلك حصرًا.
export default async function SalesOverviewPage({
  ...
  const rangeChipItems = [
    { label: "كل الفترات", href: "/sales", active: !searchParams.from },
    { label: "اليوم", href: `/sales?from=${isoDate(today)}`, active: searchParams.from === isoDate(today) },
    { label: "آخر 7 أيام", href: `/sales?from=${isoDate(weekAgo)}`, active: searchParams.from === isoDate(weekAgo) },
    { label: "آخر 30 يوم", href: `/sales?from=${isoDate(monthAgo)}`, active: searchParams.from === isoDate(monthAgo) },
  ];
