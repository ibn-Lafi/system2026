...
// الرسمية للعلامات التجارية المذكورة.
const LOGOS: { key: string; node: React.ReactNode }[] = [
  {
    key: "mcdonalds",
  ...
  },
  {
    key: "sbaah",
    node: (
      <span className={`${thmanyahSans.className} whitespace-nowrap text-2xl font-black lg:text-3xl`}>سـبـعـة</span>
    ),
  },
];
    ...
    >
      <div className="flex w-max animate-marquee items-center gap-10 lg:gap-14">
        {[...LOGOS, ...LOGOS].map((logo, index) => (
          <div key={`${logo.key}-${index}`} className="text-background">
            {logo.node}
          </div>
        ))}
      </div>
